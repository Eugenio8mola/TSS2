...
static int simulator_start_and_wait_for_ports(pid_t *simulator_pid, const tcti_sim_variant *variant, uint16_t port) {
    ...
    *simulator_pid = fork();
    ...
    while (1) {
        ...
        if (num_sockets < 0 && num_sockets != ERROR_PROCESS_DIED_MAYBE) {
            return ERROR_INTERNAL;
    ...
    TSS2_TCTI_CONTEXT *tctiContext)
{
    ...
    free(tcti_start_sim->tcti_child_name_conf);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 10; j++) {
            ...
            if (ret == EXIT_SUCCESS) {
                ...
                goto cleanup_tcti_child_name_conf;
            }
        }
        ...
        if (rc != TSS2_RC_SUCCESS) {
            ...
            LOG_ERROR ("Error loading TCTI: %s. Maybe TPM Simulator died?", conf);
            /* retry */
        } else {
            break;
        }
